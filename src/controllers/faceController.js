...
export const uploadFaceImages = async (req, res) => {
    try {
        ...
        if (!images || !Array.isArray(images) || images.length < MIN_SAMPLE_IMAGES || images.length > MAX_SAMPLE_IMAGES) {
            return apiResponse.error(res, 'Need 5-10 images.', 400);
        }
        const imageUrls = await uploadMultipleBase64Files(images, BUCKETS.FACES, `user-${userId}`);
        ...
        return apiResponse.error(res, 'Error uploading images.', 500);
    }
};
...
export const markFaceAttendance = async (req, res) => {
    try {
        ...
        if (confidenceScore < MIN_CONFIDENCE_THRESHOLD) return apiResponse.error(res, 'Low confidence.', 400);
        const imageUrl = await uploadBase64File(capturedImage, BUCKETS.ATTENDANCE, `session-${sessionId}-student-${studentId}`);
